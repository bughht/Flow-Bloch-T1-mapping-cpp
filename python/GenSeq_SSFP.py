...
class bSSFP:
    ...
    def data_process(self):
        ...
        self.GX = self.k_shape[0] / (2 * gamma * self.FOV[0] * self.TR /
                                     4) * 1e3
        self.GY_max = self.k_shape[1] / (2 * gamma * self.FOV[1] * self.TR /
                                         4) * 1e3
    ...
    def add_readout(self, t_start, FA, TR, GX, GY, kY_idx):
        ...
        GY_ts_0 = {"t": t_start, "type": "GY", "G": GY}
        GX_ts_0 = {"t": t_start, "type": "GX", "G": -GX}
        GY_ts_1 = {"t": t_start + TR / 4, "type": "GY", "G": 0}
        GX_ts_1 = {"t": t_start + TR / 4, "type": "GX", "G": GX}
        ...
        GY_tx_2 = {"t": t_start + TR / 4 + TR / 2, "type": "GY", "G": -GY}
        GX_tx_2 = {"t": t_start + TR / 4 + TR / 2, "type": "GX", "G": -GX}
        GY_tx_3 = {"t": t_start + TR, "type": "GY", "G": 0}
        GX_tx_3 = {"t": t_start + TR, "type": "GX", "G": 0}
    ...
    def generate(self, name):
        ...
        for readout_idx, readout_kY in enumerate(self.readout_kY_idx):
            ...
            self.add_readout(
                t_start,
                self.FA *
                (((readout_idx % 2) * 2 - 1) if self.sign_alter else 1),
                self.TR, self.GX,
                self.GY_max * (2 * readout_kY / self.NFlip - 1), readout_kY)
