...
def MAGIR(t, Mz):
    def T1_curve(t, T1, A, B):
        # return A-B*np.exp(-t/T1)
        return np.abs(A-B*np.exp(-t/T1))
    params = curve_fit(T1_curve, t, Mz, p0=[1, 100, 100], maxfev=int(1e5))
...
def MF_MAGIR(t, Mz):
    ...
    for i in range(len(Mz)):
        Mz_local = Mz.copy()
        Mz_local[:i] *= -1
        p_opt, p_cov = curve_fit(T1_curve, t, Mz_local, p0=[
                                 1, 100, 100], maxfev=int(1e5))
...
if __name__ == "__main__":
    ...
    Mz0 = np.array([41115.18163071, 3250.25304817, 24860.71632824, 35779.41128587,
