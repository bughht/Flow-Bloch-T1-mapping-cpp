...
def load_kspace(path, id):
    path_full = os.path.join(path, "Kspace" + str(id) + ".bin")
    with open(path_full, "rb") as f:
        data = yaml.round_trip_load(f)["mat"]
    return np.asarray(data["data"]).reshape(data["rows"], data["cols"])
...
if __name__ == "__main__":
    MOLLI_readout_img = []
    MOLLI_readout_t = np.array([200, 1200, 2200, 3200, 4200, 300, 1300, 2300
                                ]) * 1e-3
    for i in range(8):
        MOLLI_readout_img.append(
            load_img(
                "../Flow-Bloch-T1-mapping-cpp/img_MOLLI/533_TR2.8_FA10_FOV320_K64_center_first_64^3_323216",
                i))
    ...
    for x, y in tqdm(np.ndindex(MOLLI_readout_img[0].shape)):
        T1_star, T1, A, B = MF_MAGIR(MOLLI_readout_t, MOLLI_readout_img[:, x,
                                                                        y])
        T1_result[x, y] = T1
