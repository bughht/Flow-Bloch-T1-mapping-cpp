...
# flow_speed = np.linspace(0, 1, 16 * 32).reshape(32, 16).astype(str)
# flow_speed = np.linspace(0, 0.02, 2*16).reshape(2, 16).astype(str)
flow_speed = np.linspace(0.15, 0.17, 16).reshape(1, 16).astype(str)
...
for exp_idx, fs in enumerate(flow_speed):
    ...
    save_path = "exp_result_ksp/ID{}_Speed_min{}_max{}".format(
        exp_idx, fs[0], fs[-1])
